"""
    FILE LOCATON :- venv/Lib/site-packages/livs/yolo_io.py
"""
#!/usr/bin/env python
# -*- coding: utf8 -*-
import sys
...
class YOLOWriter:
    ...
    def __init__(self, folder_name, filename, img_size, database_src='Unknown', local_img_path=None):
        ...
        self.verified = False
        # self.classes_list = []
        # classes_file_path = os.path.join(os.path.dirname(os.path.abspath(target_file)), "classes.txt")
        # print("test : ",classes_file_path)
        # if os.path.exists(classes_file_path):
        #     with open(classes_file_path, 'r') as f:
        #         self.classes_list = f.read().strip('\n').split('\n')
    ...
    def bnd_box_to_yolo_line(self, box, class_list=[]):
        ...
        print("box name : ",box_name, " ; ","Index : ",class_index)
    ...
    def save(self, class_list=[], target_file=None):
        ...
        if target_file is None:
            ...
            # Check if classes.txt exists and read existing classes
        ...
        for box in self.box_list:
            ...
            class_index, x_center, y_center, w, h = self.bnd_box_to_yolo_line(box, existing_classes)
            # print (classIndex, x_center, y_center, w, h)
            out_file.write("%d %.6f %.6f %.6f %.6f\n" % (class_index, x_center, y_center, w, h))

            # Adding new box name ( New Class ) in classes.txt file
            boksu = box['name']
            if boksu not in existing_classes:
                existing_classes.append(boksu)
                out_class_file.write('\n'+boksu)
        
        print("New array ",existing_classes)

        # print("This is class_list : ", class_list)
        # for c in class_list:
